...
export function AvatarEditor({ avatarUrl, name }: { avatarUrl: string; name: string }) {
  ...
  return <>
      ...
      <div><p className="text-sm font-semibold text-apple-ink">Profile photo</p><p className="mt-1 max-w-md text-xs leading-5 text-apple-ink-muted-48">JPG, PNG, or WebP. Up to 5 MB and at least 128 × 128 pixels.</p>
        <div className="mt-3 flex flex-wrap gap-2">
          <LoadingButton
            ...
            className="h-9 rounded-xl bg-zinc-900 px-3 text-xs font-medium text-white hover:bg-zinc-800"
          >
          ...
          {avatarUrl && (
            <LoadingButton
              ...
              className="h-9 rounded-xl border-zinc-200 px-3 text-xs font-medium text-red-600 hover:bg-red-50"
            >
              Remove
            </LoadingButton>
          )}
    ...
    {source && (
      <Modal open title="Crop photo" onClose={() => setSource(null)} size="sm" loading={isLoading}>
        <p className="text-xs text-apple-ink-muted-48">Adjust the framing before uploading.</p>
        <div className="mx-auto mt-5 h-64 w-64 overflow-hidden rounded-full bg-zinc-100 ring-4 ring-zinc-100">
          <img
            src={source}
            alt="Crop preview"
            className="h-full w-full object-cover"
            style={{ transform: `scale(${zoom})`, objectPosition: `${positionX}% ${positionY}%` }}
          />
        </div>
        <div className="mt-5 space-y-3">
          {[
            ...
            ["Vertical", positionY, 0, 100, 1, setPositionY],
          ].map(([label, value, min, max, step, setter]) => (
            <label
              key={String(label)}
              className="grid grid-cols-[72px_1fr] items-center gap-3 text-xs font-medium text-apple-ink-muted-80"
            >
        ...
        <div className="mt-6 flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={() => setSource(null)} disabled={isLoading}>
            Cancel
          </Button>
          <Button
            type="button"
