...
function PlanCard({ plan }: { plan: Plan }) {
  ...
  return <article className={cn("relative flex h-full flex-col rounded-lg border bg-apple-canvas p-6 transition-colors", plan.isRecommended ? "border-blue-300 ring-2 ring-blue-500/15" : "border-apple-hairline") }>
    <div className="absolute -top-3 left-5 flex gap-2">{plan.isRecommended && <span className="rounded-full bg-blue-600 px-3 py-1 text-[10px] font-bold text-white">Recommended</span>}{plan.isPopular && <span className="rounded-full bg-violet-600 px-3 py-1 text-[10px] font-bold text-white">Most popular</span>}</div>
    ...
    <Link href={action.href} className={cn("mt-6 rounded-xl px-4 py-3 text-center text-sm font-semibold transition", plan.isRecommended ? "bg-blue-600 text-white hover:bg-blue-700" : "border border-apple-hairline text-zinc-800 hover:bg-apple-canvas-parchment")}>{action.label}</Link>
    <div className="mt-6 border-t border-apple-divider-soft pt-5"><p className="text-[11px] font-bold uppercase tracking-wider text-apple-ink-muted-48">Included features</p><ul className="mt-3 space-y-2">{planFeatures.length ? planFeatures.map((feature) => <li key={feature} className="flex gap-2 text-xs text-apple-ink-muted-80"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-blue-600" />{feature}</li>) : <li className="text-xs text-apple-ink-muted-48">Configured by your plan administrator.</li>}</ul></div>
    {planLimits.length > 0 && <div className="mt-5 border-t border-apple-divider-soft pt-5"><p className="text-[11px] font-bold uppercase tracking-wider text-apple-ink-muted-48">Plan limits</p><ul className="mt-3 space-y-2">{planLimits.map((limit) => <li key={limit} className="text-xs text-apple-ink-muted-80">{limit}</li>)}</ul></div>}
  </article>;
}
