...
export function ProfileDropdown({ compact = false }: { compact?: boolean }) {
  ...
  return <div ref={ref} className="relative">
    <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={language === "bn" ? "প্রোফাইল মেনু খুলুন" : "Open profile menu"} className={cn("flex items-center gap-1 rounded-full outline-none ring-offset-2 focus-visible:ring-2 focus-visible:ring-apple-ink", compact && "rounded-sm border border-apple-hairline py-1 pl-1 pr-2")}><UserAvatar showChevron={compact}/></button>
    <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: -6, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: .98 }} transition={{ duration: .16 }} className="absolute right-0 top-[calc(100%+10px)] z-[70] w-64 overflow-hidden rounded-lg border border-apple-hairline bg-apple-canvas p-1.5 shadow-xl">
      <div className="mb-1 flex items-center gap-3 rounded-lg bg-apple-canvas-parchment px-3 py-3"><UserAvatar className="h-10 w-10"/><div className="min-w-0"><p className="truncate text-sm font-semibold text-apple-ink">{profile?.name || fallback?.name || (language === "bn" ? "ইউজার" : "User")}</p><p className="truncate text-xs text-apple-ink-muted-48">{profile?.email || fallback?.email}</p></div></div>
      ...
      <Button
        ...
        className="h-auto w-full justify-start gap-2.5 rounded-lg border-t border-apple-divider-soft px-3 py-2.5 text-sm text-red-600 hover:bg-red-50 hover:text-red-600 font-medium"
      >
