...
import {
  ...
  RefreshCw, ExternalLink, Clock, MapPin, Loader2,
} from "lucide-react";
...
export default function LiveVisitorsPage() {
  ...
  const { data, isLoading: liveLoading, refetch } = useGetLiveVisitorsQuery(storeId!, {
    skip: !storeId,
  ...
  if (liveLoading) return <AnalyticsLoading />;
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-apple-ink">Live Visitors</h1>
      ...
      {count === 0 ? (
        <AnalyticsEmptyState icon={Users} title="No visitors online right now" description="Live data will appear here once visitors are browsing your store." />
      ) : (
        <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-zinc-50">
                  {visitors.map((v) => {
                    ...
                    return (
                      <tr key={String(v._id)} className="hover:bg-apple-canvas-parchment/50 transition-colors">
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-mono text-apple-ink-muted-48">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-apple-ink-muted-80 truncate max-w-[200px]">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-1.5">
                            {String(v.device) === "Mobile" ? <Smartphone className="h-3 w-3 text-apple-ink-muted-48" /> :
                             String(v.device) === "Tablet" ? <Tablet className="h-3 w-3 text-apple-ink-muted-48" /> :
                             <Monitor className="h-3 w-3 text-apple-ink-muted-48" />}
                            <span className="text-xs text-apple-ink-muted-80">{String(v.device ?? "—")}</span>
                          </div>
                        </td>
                        <td className="px-5 py-3 text-xs text-apple-ink-muted-80">{String(v.browser ?? "—")}</td>
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-1">
                            <MapPin className="h-3 w-3 text-apple-ink-muted-48" />
                            <span className="text-xs text-apple-ink-muted-80">{String(v.country ?? "—")}</span>
                          </div>
                        </td>
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-1">
                            <Clock className="h-3 w-3 text-apple-ink-muted-48" />
                            <span className="text-xs text-apple-ink-muted-80">
                              {duration < 60 ? `${duration}s` : `${Math.floor(duration / 60)}m ${duration % 60}s`}
                            </span>
                          </div>
                        </td>
                        <td className="px-5 py-3 text-xs text-apple-ink-muted-48 max-w-[120px] truncate">
            ...
            {[
              {
                ...
                data: [...new Set(visitors.map((v) => String(v.device ?? "Unknown")))].map((d) => ({
                  name: d, count: visitors.filter((v) => String(v.device ?? "Unknown") === d).length,
                })),
                icon: Monitor,
              },
              {
                title: "Active Pages",
                data: [...new Set(visitors.map((v) => String(v.currentPage ?? v.path ?? "Unknown")))].map((p) => ({
                  name: p, count: visitors.filter((v) => String(v.currentPage ?? v.path ?? "Unknown") === p).length,
                })),
                icon: ExternalLink,
              },
              {
                ...
                data: [...new Set(visitors.map((v) => String(v.country ?? "Unknown")))].map((c) => ({
                  name: c, count: visitors.filter((v) => String(v.country ?? "Unknown") === c).length,
                })),
                icon: MapPin,
              },
            ].map((section, si) => (
              <motion.div key={section.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + si * 0.05 }}
                ...
                <div className="space-y-2">
                  {section.data.map((d) => {
                    const pct = visitors.length > 0 ? Math.round((d.count / visitors.length) * 100) : 0;
                    return (
                      <div key={d.name}>
