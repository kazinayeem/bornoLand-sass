import type { TrackingProvider, StandardEventName, TrackingPayload, PublicTikTokPixelConfig } from "./types";
...
export class TikTokPixelProvider implements TrackingProvider {
  readonly name = "tiktok" as const;
  private initialized = false;
  private pixelId = "";
  private testEventCode = "";

  init(config: PublicTikTokPixelConfig): void {
  ...
  track(eventName: StandardEventName, payload: TrackingPayload = {}, eventId?: string): void {
    ...
    try {
      ...
      console.warn("[TikTokPixelProvider] Failed to fire event:", eventName, err);
    }
  }
