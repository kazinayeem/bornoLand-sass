    ...
    <aside className="flex h-full flex-col border-r border-zinc-200/80 bg-white">
      ...
      <div className="border-b border-zinc-100 px-4 py-4">
        ...
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          <span className={cn("rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider", store.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-zinc-100 text-zinc-500")}>
            {store.status}
          </span>
      ...
      <div className="border-b border-zinc-100 px-4 py-3">
        <div className="relative">
          ...
          <input
            ...
            className="h-8 w-full rounded-lg border border-zinc-200 bg-zinc-50 pl-8 pr-3 text-xs outline-none transition-colors focus:border-blue-300 focus:bg-white focus:ring-2 focus:ring-blue-500/20"
          />
        </div>
      </div>
                ...
                <div className="mt-0.5 space-y-0.5">
                  {group.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <item.icon className={cn("h-3.5 w-3.5 shrink-0", isActive ? "text-white" : "text-zinc-400")} />
      ...
      <div className="border-t border-zinc-100 px-3 py-3">
        <button
          ...
          className="flex w-full items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 px-3 py-2 text-xs font-semibold text-white shadow-sm transition-all hover:shadow hover:brightness-110"
        >
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-1.5">
          <button
            onClick={onSettings}
            className="flex items-center justify-center gap-1.5 rounded-lg border border-zinc-200 px-2 py-1.5 text-[11px] font-medium text-zinc-600 transition-colors hover:bg-zinc-50"
          >
