...
export default function PagesPage() {
  ...
  if (topPages.length === 0 && topPagesFromCharts.length === 0) {
    return (
      <>
        <div>
          <h1 className="text-xl font-bold text-zinc-900">Pages</h1>
          <p className="text-sm text-zinc-500">Most viewed pages on your store</p>
        </div>
  ...
  return (
    <>
      <div>
        <h1 className="text-xl font-bold text-zinc-900">Pages</h1>
        <p className="text-sm text-zinc-500">Most viewed pages on your store — this month</p>
      </div>
            ...
            {topLanding.map((p, i) => (
              <motion.div key={String(p.url ?? i)} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <span className="truncate text-zinc-700">{String(p.title ?? p.url ?? p.path ?? "Unknown")}</span>
                </div>
                <span className="ml-3 shrink-0 font-semibold text-zinc-900">{String(p.views ?? 0)} views</span>
              </motion.div>
            ))}
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <AnalyticsChartCard title="Most Viewed Products" delay={0.15}>
          ...
          ) : <p className="text-xs text-zinc-400 py-4">No product view data yet</p>}
        </AnalyticsChartCard>
            ...
            <div className="space-y-2">
              {((topContent?.topCategories ?? []) as Array<Record<string, unknown>>).slice(0, 10).map((c, i) => (
                <div key={String(c.categoryId ?? i)} className="flex items-center justify-between text-xs">
                  <span className="truncate text-zinc-700">{String(c.name ?? "Unknown")}</span>
                  <span className="ml-2 shrink-0 font-medium text-zinc-900">{String(c.views ?? 0)} views</span>
                </div>
              ))}
            </div>
          ) : <p className="text-xs text-zinc-400 py-4">No category view data yet</p>}
        </AnalyticsChartCard>
      </div>
    </>
  );
