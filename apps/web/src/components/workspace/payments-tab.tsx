...
import {
  CreditCard, Plus, Loader2, Pencil, Trash2,
  Eye, EyeOff, X, Check, Smartphone, Landmark, Banknote, Globe,
} from "lucide-react";
...
export function PaymentsTab({ storeId }: PaymentsTabProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deletePayment({ storeId, id }).unwrap();
    ...
    <div className="space-y-4">
      ...
      {methods.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-12 text-center">
          ...
          <h3 className="mt-3 text-base font-semibold text-zinc-900">No payment methods</h3>
          <p className="mt-1 text-sm text-zinc-500">Add payment methods to accept payments at checkout.</p>
        </div>
      ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-semibold text-zinc-900">{m.label}</h4>
                        <span className="text-[10px] uppercase font-semibold text-zinc-400">{m.type}</span>
                      </div>
                      <p className="text-xs text-zinc-500 truncate">{m.accountNumber}</p>
                      {m.instructions && <p className="text-xs text-zinc-400 truncate mt-0.5">{m.instructions}</p>}
                    </div>
                  </div>
                  <button onClick={() => toggleEnabled(m)}
                    className={`rounded-lg px-2.5 py-1 text-xs font-medium transition-colors ${
                      m.enabled ? "bg-emerald-50 text-emerald-700 hover:bg-emerald-100" : "bg-zinc-100 text-zinc-400 hover:bg-zinc-200"
                    }`}>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm p-4 pt-20"
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-semibold text-zinc-900">{editMethod ? "Edit" : "Add"} Payment Method</h3>
