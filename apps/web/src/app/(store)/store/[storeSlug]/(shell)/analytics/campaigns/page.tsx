...
export default function CampaignsPage() {
  ...
  if (campaigns.length === 0) {
    return (
      <>
        <div>
          <h1 className="text-xl font-bold text-apple-ink">Campaigns</h1>
          <p className="text-sm text-apple-ink-muted-48">UTM campaign performance tracking</p>
        </div>
            ...
            {campaigns.map((c) => (
              <div key={String(c._id)} className="flex items-center justify-between rounded-lg bg-apple-canvas-parchment p-3 text-xs">
                ...
                <div className="flex items-center gap-4 shrink-0 ml-3">
                  <span className="text-apple-ink-muted-48">{String(c.visits ?? 0)} visits</span>
