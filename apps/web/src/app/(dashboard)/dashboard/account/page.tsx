...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return <div className="space-y-1.5"><Label>{label}</Label>{children}{hint && <p className="text-xs text-zinc-500">{hint}</p>}</div>;
}
...
function Toggle({ checked, onChange, label, description, icon: Icon }: { checked: boolean; onChange: (checked: boolean) => void; label: string; description: string; icon: typeof Bell }) {
  return <div className="flex items-center gap-3 rounded-2xl border border-zinc-200/80 p-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zinc-100 text-zinc-600"><Icon className="h-4 w-4"/></div><div className="min-w-0 flex-1"><p className="text-sm font-medium text-zinc-900">{label}</p><p className="text-xs leading-5 text-zinc-500">{description}</p></div><button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)} className={`relative h-6 w-11 rounded-full transition ${checked ? "bg-zinc-950" : "bg-zinc-200"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${checked ? "left-6" : "left-1"}`}/></button></div>;
}
...
export default function AccountSettingsPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-7 pb-16">
    ...
    <section className="overflow-hidden rounded-3xl border border-zinc-200/80 bg-white shadow-sm"><div className="border-b border-zinc-100 bg-gradient-to-r from-zinc-50 to-white px-5 py-4 sm:px-6"><div className="flex items-center gap-2"><UserRound className="h-4 w-4 text-zinc-500"/><h2 className="font-semibold text-zinc-950">Personal profile</h2></div><p className="mt-1 text-sm text-zinc-500">This information identifies you across your workspace.</p></div><div className="space-y-7 p-5 sm:p-6">
      <AvatarEditor avatarUrl={profile?.avatarUrl ?? ""} name={form.name || "User"}/>
      <div className="grid gap-5 sm:grid-cols-2"><Field label="Full name"><Input value={form.name} onChange={(e) => change("name", e.target.value)} placeholder="Your full name"/></Field><Field label="Username" hint="3–30 characters: letters, numbers, dots, hyphens, or underscores."><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">@</span><Input value={form.username} onChange={(e) => change("username", e.target.value.toLowerCase())} className="pl-8"/></div></Field>
    ...
    <section id="preferences" className="scroll-mt-24 overflow-hidden rounded-3xl border border-zinc-200/80 bg-white shadow-sm"><div className="border-b border-zinc-100 px-5 py-4 sm:px-6"><div className="flex items-center gap-2"><Globe2 className="h-4 w-4 text-zinc-500"/><h2 className="font-semibold text-zinc-950">Dashboard preferences</h2></div><p className="mt-1 text-sm text-zinc-500">Customize appearance, dates, and communications.</p></div><div className="grid gap-5 p-5 sm:grid-cols-3 sm:p-6">
