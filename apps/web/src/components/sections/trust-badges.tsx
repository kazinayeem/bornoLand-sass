    ...
    return (
      <SectionWrapper section={section}>
        <div className="px-4 sm:px-6 lg:px-8 py-12 text-center">
          ...
          <p className="mt-4 text-sm text-apple-ink-muted-48">No trust badges yet. Add them in the Content tab.</p>
        </div>
      </SectionWrapper>
    );
  }
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {visible.map((badge) => (
            <div key={badge.id} className="flex flex-col items-center rounded-2xl border border-zinc-200 bg-white p-6 text-center shadow-sm">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-apple-canvas-parchment text-xl">
                {badge.icon || "🔒"}
              </div>
              <h3 className="mt-3 text-sm font-semibold text-apple-ink">{badge.title || "Badge"}</h3>
              <p className="mt-1 text-xs text-apple-ink-muted-48">{badge.description || ""}</p>
            </div>
          ))}
        </div>
      </div>
