...
import { Plus, Loader2, GripVertical, Pencil, Trash2, X, Check, MessageSquare } from "lucide-react";
...
export function CmsFaqsEditor() {
  ...
  const { data: faqsData, isLoading: faqsLoading } = useGetFaqsQuery(storeId ?? "", { skip: !storeId });
  ...
  const handleCreate = async () => {
    ...
    try {
      await createFaq({ storeId: storeId!, data: { question, answer, category: category || undefined } }).unwrap();
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateFaq({ storeId: storeId!, faqId: editId, data: { question, answer, category: category || undefined } }).unwrap();
  ...
  const handleDelete = async (faqId: string) => {
    ...
    try {
      await deleteFaq({ storeId: storeId!, faqId }).unwrap();
  ...
  const handleToggleActive = async (faq: FaqItem) => {
    try {
      await updateFaq({ storeId: storeId!, faqId: faq._id, data: { active: !faq.active } }).unwrap();
    } catch {
      toast.error("Failed to update FAQ");
    }
  };
  ...
  const handleMoveUp = async (index: number) => {
    ...
    try {
      await reorderFaqs({ storeId: storeId!, orderedIds: reordered.map((f) => f._id) }).unwrap();
