...
export function AvatarEditor({ avatarUrl, name }: { avatarUrl: string; name: string }) {
  ...
  return <>
    <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
      ...
      <div><p className="text-sm font-semibold text-zinc-900">Profile photo</p><p className="mt-1 max-w-md text-xs leading-5 text-zinc-500">JPG, PNG, or WebP. Up to 5 MB and at least 128 × 128 pixels.</p>
    ...
    {source && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Crop profile photo"><div className="w-full max-w-md rounded-3xl border border-zinc-200 bg-white p-5 shadow-2xl">
      <div className="flex items-center justify-between"><div><h2 className="font-semibold text-zinc-950">Crop photo</h2><p className="text-xs text-zinc-500">Adjust the framing before uploading.</p></div><button type="button" onClick={() => setSource(null)} className="rounded-xl p-2 text-zinc-500 hover:bg-zinc-100"><X className="h-4 w-4"/></button></div>
      ...
      <div className="mt-5 space-y-3">{[["Zoom", zoom, 1, 3, .05, setZoom], ["Horizontal", positionX, 0, 100, 1, setPositionX], ["Vertical", positionY, 0, 100, 1, setPositionY]].map(([label, value, min, max, step, setter]) => <label key={String(label)} className="grid grid-cols-[72px_1fr] items-center gap-3 text-xs font-medium text-zinc-600"><span>{String(label)}</span><input type="range" value={Number(value)} min={Number(min)} max={Number(max)} step={Number(step)} onChange={(event) => (setter as (value: number) => void)(Number(event.target.value))} className="accent-zinc-900"/></label>)}</div>
      <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setSource(null)} className="h-10 rounded-xl border border-zinc-200 px-4 text-sm font-medium text-zinc-700">Cancel</button><button type="button" disabled={isLoading} onClick={async () => { try { await createCrop(); } catch { toast.error("Could not upload photo"); } }} className="inline-flex h-10 items-center gap-2 rounded-xl bg-zinc-950 px-4 text-sm font-medium text-white">{isLoading && <Loader2 className="h-4 w-4 animate-spin"/>}Save photo</button></div>
    </div></div>}
  </>;
}
