  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          <h2 className="text-xl font-bold text-apple-ink">Payment Methods</h2>
          <p className="mt-0.5 text-sm text-apple-ink-muted-48">Manage bKash, Nagad, COD, and more.</p>
        </div>
      ...
      {showForm && (
        <motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-apple-ink">{editing ? "Edit" : "Add"} Payment Method</h3>
            <button type="button" onClick={resetForm} className="rounded p-0.5 text-apple-ink-muted-48 hover:text-apple-ink-muted-80">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Type</label>
              <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as PaymentMethodData["type"] })}
                className="h-9 w-full rounded-lg border border-zinc-200 bg-white px-2.5 text-xs text-apple-ink-muted-80 focus:border-zinc-400 focus:outline-none">
                {typeOptions.map((o) => (
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Label</label>
              <input type="text" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })}
                required placeholder="e.g. bKash Personal"
                className="h-9 w-full rounded-lg border border-zinc-200 bg-white px-2.5 text-xs text-apple-ink-muted-80 placeholder:text-apple-ink-muted-48 focus:border-zinc-400 focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Account Number</label>
              <input type="text" value={form.accountNumber} onChange={(e) => setForm({ ...form, accountNumber: e.target.value })}
                placeholder="e.g. 017XXXXXXXX"
                className="h-9 w-full rounded-lg border border-zinc-200 bg-white px-2.5 text-xs text-apple-ink-muted-80 placeholder:text-apple-ink-muted-48 focus:border-zinc-400 focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Account Type</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Payment Instructions</label>
            <textarea value={form.instructions} onChange={(e) => setForm({ ...form, instructions: e.target.value })}
              rows={2} placeholder="e.g. Send money to this number and enter transaction ID during checkout"
              className="w-full rounded-lg border border-zinc-200 bg-white px-2.5 py-2 text-xs text-apple-ink-muted-80 placeholder:text-apple-ink-muted-48 focus:border-zinc-400 focus:outline-none" />
          </div>

          <div className="flex justify-end gap-2">
            <button type="button" onClick={resetForm}
              className="rounded-lg border border-zinc-200 px-3.5 py-1.5 text-xs font-medium text-apple-ink-muted-80 hover:bg-apple-canvas-parchment">
              Cancel
            </button>
            <button type="submit"
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 py-12">
          ...
          <p className="text-sm text-apple-ink-muted-48">No payment methods configured</p>
          <button onClick={() => setShowForm(true)}
            className="mt-2 text-xs font-medium text-apple-ink underline underline-offset-2">
        ...
        <div className="space-y-2">
          {methods.map((m) => (
            <div key={m._id}
              ...
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-apple-canvas-parchment text-apple-ink-muted-48">
                {getTypeIcon(m.type)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-apple-ink">{m.label}</p>
              ...
              <button onClick={() => toggleEnabled(m)} title={m.enabled ? "Disable" : "Enable"}
                className="rounded-lg p-1.5 text-apple-ink-muted-48 hover:text-apple-ink-muted-80">
                {m.enabled ? <ToggleRight className="h-4 w-4 text-green-500" /> : <ToggleLeft className="h-4 w-4" />}
              </button>
              <button onClick={() => startEdit(m)}
                className="rounded-lg px-2.5 py-1 text-[11px] font-medium text-apple-ink-muted-48 hover:bg-apple-canvas-parchment">
                Edit
              </button>
