/**
 * Bangladesh Administrative Location Dataset (Client & SSR)
 * Contains 8 divisions, 64 districts, and upazilas/thanas with English and Bengali names.
 */
...
/** Normalizes text for bilingual search */
...
/** Get upazilas filtered by district */
...
export function getUnions(upazilaId?: string): Array<{ id: string; name: string; nameBn: string }> {
  ...
  const upz = BANGLADESH_UPAZILAS.find((u) => u.id.toLowerCase() === norm);
  return upz?.unions ?? [];
}

/** Find division by id, name, or nameBn */
export function findDivision(idOrName?: string): Division | undefined {
  if (!idOrName) return undefined;
  const norm = normalizeSearch(idOrName);
  return BANGLADESH_DIVISIONS.find(
    (d) =>
      normalizeSearch(d.id) === norm ||
      normalizeSearch(d.name) === norm ||
      normalizeSearch(d.nameBn) === norm
  );
}

/** Find district by id, name, or nameBn */
...
/** Find upazila by id, name, or nameBn */
export function findUpazila(idOrName?: string): Upazila | undefined {
  if (!idOrName) return undefined;
  const norm = normalizeSearch(idOrName);
  return BANGLADESH_UPAZILAS.find(
    (u) =>
      normalizeSearch(u.id) === norm ||
      normalizeSearch(u.name) === norm ||
      normalizeSearch(u.nameBn) === norm
  );
}
