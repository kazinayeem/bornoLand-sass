    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-12 text-center">
          ...
          <h3 className="mt-3 text-lg font-semibold text-zinc-900">No invoices found</h3>
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-xs font-semibold text-zinc-900">{inv.invoiceNumber}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm text-zinc-700">{storeName}</td>
                    <td className="px-4 py-3">
                      <span className="inline-flex rounded-lg bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-700">
                        {planName}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-xs text-zinc-500">{formatDate(inv.createdAt)}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="inline-flex items-center gap-1">
                        <button
                          ...
                          className="rounded-lg border border-zinc-200 bg-white p-1.5 text-zinc-500 hover:bg-zinc-50"
                          title="View Details"
                        >
            ...
            <div className="flex items-center justify-between border-t border-zinc-100 px-4 py-3">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={page <= 1}
                  className="rounded-lg border border-zinc-200 bg-white p-1.5 text-zinc-500 hover:bg-zinc-50 disabled:opacity-40"
                >
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <span className="px-3 text-xs font-medium text-zinc-700">{page} / {totalPages}</span>
                <button
      ...
      >
        ...
        <div className="max-h-[60vh] overflow-y-auto p-6 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-zinc-500">{formatDate(invoice.createdAt)}</span>
          </div>

          {/* Details Grid */}
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-zinc-50 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Plan</p>
              <p className="mt-0.5 text-sm font-semibold text-zinc-900">{planName}</p>
            </div>
            <div className="rounded-xl bg-zinc-50 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Amount</p>
              <p className="mt-0.5 text-sm font-semibold text-zinc-900">{formatBDT(invoice.total)}</p>
            </div>
            <div className="rounded-xl bg-zinc-50 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Customer</p>
              <p className="mt-0.5 text-sm font-semibold text-zinc-900">{customerName}</p>
              <p className="text-xs text-zinc-500">{customerEmail}</p>
            </div>
            <div className="rounded-xl bg-zinc-50 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Gateway</p>
              <p className="mt-0.5 text-sm font-semibold text-zinc-900 capitalize">{invoice.gateway || "—"}</p>
            </div>
            {invoice.transactionId && (
              <div className="col-span-2 rounded-xl bg-zinc-50 p-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Transaction ID</p>
                <p className="mt-0.5 font-mono text-sm font-semibold text-zinc-900">{invoice.transactionId}</p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400 mb-1">Verification URL</p>
            <p className="text-xs text-blue-600 break-all">{verificationUrl}</p>
          </div>

          {/* Status Change */}
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-400">Change Status</p>
            <div className="flex gap-2">
