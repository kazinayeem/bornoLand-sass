    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={addItem}
          className="inline-flex items-center gap-1 rounded-lg border border-zinc-200 px-2 py-1 text-[10px] font-medium text-zinc-600 hover:bg-zinc-50"
        >
      ...
      {items.length === 0 && (
        <p className="py-3 text-center text-[11px] text-zinc-400">No items yet. Click &quot;{addLabel}&quot; to add one.</p>
      )}
        ...
        {items.map((item, index) => (
          <div key={item.id || index} className="rounded-lg border border-zinc-200 bg-white">
            <div className="flex items-center gap-1 border-b border-zinc-100 px-2 py-1.5">
              ...
              <span className="flex-1 truncate text-[10px] font-medium text-zinc-600">
            ...
            <div className="space-y-1.5 p-2">
              {fields.map((field) => (
                <div key={field.key}>
                  <label className="mb-0.5 block text-[9px] font-medium text-zinc-500">{field.label}</label>
                  {field.type === "textarea" ? (
                    <textarea
                      ...
                      rows={2}
                      className="w-full resize-none rounded-md border border-zinc-200 bg-zinc-50 px-2 py-1 text-[10px] text-zinc-700 placeholder:text-zinc-300 focus:border-zinc-400 focus:outline-none"
                    />
                  ) : (
                    <input
                      ...
                      onChange={(e) => updateField(index, field.key, e.target.value)}
                      placeholder={field.placeholder}
                      className="w-full rounded-md border border-zinc-200 bg-zinc-50 px-2 py-1 text-[10px] text-zinc-700 placeholder:text-zinc-300 focus:border-zinc-400 focus:outline-none"
                    />
                  )}
                </div>
              ))}
