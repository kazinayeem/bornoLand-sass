    ...
    <div className="fixed inset-0 z-[200] grid place-items-start overflow-y-auto p-4 pt-[10vh] sm:pt-[14vh]" role="dialog" aria-modal="true" aria-label="Builder command palette">
      ...
      <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-lg border border-apple-hairline bg-apple-canvas animate-in fade-in zoom-in-95 duration-150" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="max-h-[min(56vh,520px)] overflow-y-auto p-2">
          {results.slice(0, 50).map((action) => <button key={action.id} type="button" onClick={() => { action.run(); close(); }} className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left transition hover:bg-apple-canvas-parchment"><span><span className="block text-sm font-medium text-zinc-800">{action.label}</span><span className="mt-0.5 block text-xs text-apple-ink-muted-48">{action.hint}</span></span><span className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] text-apple-ink-muted-48">↵</span></button>)}
          {!results.length && <div className="px-4 py-10 text-center"><p className="text-sm font-medium text-apple-ink-muted-80">No results</p><p className="mt-1 text-xs text-apple-ink-muted-48">Try a page, section, template, media item, or command.</p></div>}
        </div>
      </div>
    </div>,
    document.body,
