...
export function TrialBanner({ store }: { store: Store }) {
  ...
  if (status === "trial") {
    ...
    return (
      <div className="flex items-center justify-between gap-4 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
        <p>
  ...
  if (status === "expired" || status === "pending_payment") {
    return (
      <div className="flex items-center justify-between gap-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
