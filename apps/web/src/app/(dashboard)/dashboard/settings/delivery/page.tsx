  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          <h2 className="text-xl font-bold text-apple-ink">Delivery Zones</h2>
          <p className="mt-0.5 text-sm text-apple-ink-muted-48">Set delivery charges by area.</p>
        </div>
      ...
      {showForm && (
        <motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-apple-ink">{editing ? "Edit" : "Add"} Delivery Zone</h3>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Area Name</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Delivery Charge (BDT)</label>
              <input type="number" min={0} value={form.charge} onChange={(e) => setForm({ ...form, charge: Number(e.target.value) })}
                required
                className="h-9 w-full rounded-lg border border-zinc-200 bg-white px-2.5 text-xs text-apple-ink-muted-80 placeholder:text-apple-ink-muted-48 focus:border-zinc-400 focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-apple-ink-muted-48">Est. Delivery Time</label>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 py-12">
          ...
          <p className="text-sm text-apple-ink-muted-48">No delivery zones configured</p>
          ...
          {zones.map((z) => (
            <div key={z._id}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-apple-ink">{z.name}</p>
