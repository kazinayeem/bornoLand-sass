...
function PlanCard({ plan }: { plan: Plan }) {
  ...
  return <article className={cn("relative flex h-full flex-col rounded-3xl border bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl", plan.isRecommended ? "border-blue-300 ring-2 ring-blue-500/15" : "border-zinc-200") }>
    ...
    <h3 className="text-lg font-bold text-zinc-950">{plan.name}</h3><p className="mt-2 min-h-10 text-sm leading-5 text-zinc-500">{plan.description || "A flexible plan for building and growing your store."}</p>
    <div className="mt-6 flex items-baseline gap-1"><span className="text-3xl font-bold tracking-tight text-zinc-950">{formatPrice(plan)}</span>{!plan.isCustomPrice && formatPrice(plan) !== "Free" && <span className="text-xs text-zinc-400">/ month</span>}</div>
    {(plan.pricing?.yearly || plan.priceYearly) ? <p className="mt-1 text-xs text-zinc-500">Yearly: {formatPrice(plan, true)}</p> : null}
    ...
    <Link href={action.href} className={cn("mt-6 rounded-xl px-4 py-3 text-center text-sm font-semibold transition", plan.isRecommended ? "bg-blue-600 text-white hover:bg-blue-700" : "border border-zinc-200 text-zinc-800 hover:bg-zinc-50")}>{action.label}</Link>
    <div className="mt-6 border-t border-zinc-100 pt-5"><p className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">Included features</p><ul className="mt-3 space-y-2">{planFeatures.length ? planFeatures.map((feature) => <li key={feature} className="flex gap-2 text-xs text-zinc-600"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-blue-600" />{feature}</li>) : <li className="text-xs text-zinc-400">Configured by your plan administrator.</li>}</ul></div>
    {planLimits.length > 0 && <div className="mt-5 border-t border-zinc-100 pt-5"><p className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">Plan limits</p><ul className="mt-3 space-y-2">{planLimits.map((limit) => <li key={limit} className="text-xs text-zinc-600">{limit}</li>)}</ul></div>}
  </article>;
}
...
export function Pricing() {
  ...
  return <section id="pricing" className="relative px-4 py-20 sm:px-6 sm:py-28 lg:px-8"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="Pricing" title="Plans that grow with your business" description="Every price, feature, trial, and limit is configured by BornoLand administrators and shown here in real time." />
    {isLoading ? <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3"><PricingSkeleton /><PricingSkeleton /><PricingSkeleton /></div> : isError ? <div className="mx-auto mt-12 max-w-lg rounded-3xl border border-red-100 bg-red-50 p-8 text-center"><CircleX className="mx-auto h-8 w-8 text-red-500" /><h3 className="mt-3 font-semibold text-zinc-900">Pricing is temporarily unavailable</h3><p className="mt-1 text-sm text-zinc-500">Please refresh the page or contact our team for current plan details.</p></div> : plans.length === 0 ? <div className="mx-auto mt-12 max-w-lg rounded-3xl border border-zinc-200 bg-zinc-50 p-9 text-center"><Sparkles className="mx-auto h-8 w-8 text-blue-600" /><h3 className="mt-3 text-lg font-semibold text-zinc-900">New plans are coming soon</h3><p className="mt-1 text-sm text-zinc-500">Our team is preparing the right options for your store.</p></div> : <><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{plans.map((plan) => <PlanCard key={plan._id} plan={plan} />)}</div><div className="mt-14 overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm"><div className="border-b border-zinc-100 px-6 py-5"><h3 className="font-semibold text-zinc-900">Compare plan capabilities</h3><p className="mt-1 text-xs text-zinc-500">Generated from the active plan configurations.</p></div><div className="overflow-x-auto"><table className="min-w-[720px] w-full text-left text-xs"><thead className="bg-zinc-50"><tr><th className="px-6 py-3 font-semibold text-zinc-500">Feature or limit</th>{plans.map((plan) => <th key={plan._id} className={cn("px-4 py-3 font-semibold", plan.isRecommended ? "bg-blue-50 text-blue-700" : "text-zinc-500")}>{plan.name}</th>)}</tr></thead><tbody>{featureRows.map((feature) => <tr key={feature} className="border-t border-zinc-100"><th className="px-6 py-3 font-medium text-zinc-800">{feature}</th>{plans.map((plan) => { const available = enabled(plan).includes(feature) || limits(plan).includes(feature); return <td key={plan._id} className={cn("px-4 py-3", plan.isRecommended && "bg-blue-50/40")}>{available ? <Check className="h-4 w-4 text-emerald-600" /> : <CircleX className="h-4 w-4 text-zinc-300" />}</td> })}</tr>)}</tbody></table></div></div></>}
  </div></section>;
}

function PricingSkeleton() { return <div className="h-[440px] animate-pulse rounded-3xl border border-zinc-200 bg-zinc-50 p-6"><div className="h-5 w-24 rounded bg-zinc-200" /><div className="mt-4 h-10 w-36 rounded bg-zinc-200" /><div className="mt-8 space-y-3">{[1, 2, 3, 4, 5].map((item) => <div key={item} className="h-3 rounded bg-zinc-200" />)}</div></div>; }
