    ...
    <div className="space-y-6">
          ...
          <motion.div key={card.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-lg font-bold text-apple-ink">{card.value}</p>
              </div>
            </div>
          </motion.div>
        ))}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <h3 className="mb-1 text-lg font-semibold text-apple-ink">Monthly Revenue (৳)</h3>
          <p className="mb-6 text-sm text-apple-ink-muted-48">Revenue and order trends over the last 12 months</p>
          <div className="h-72">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
          className="rounded-2xl border border-zinc-200 bg-white p-6">
          <h3 className="mb-1 text-lg font-semibold text-apple-ink">Daily Revenue (৳)</h3>
          <p className="mb-6 text-sm text-apple-ink-muted-48">Last 30 days</p>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        className="rounded-2xl border border-zinc-200 bg-white p-6">
        <h3 className="mb-1 text-lg font-semibold text-apple-ink">Revenue by Plan</h3>
        <p className="mb-4 text-sm text-apple-ink-muted-48">How much each plan has generated in ৳ BDT</p>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {byPlan.map((plan) => {
            ...
            return (
              <div key={String(plan.planId ?? "")} className="rounded-xl border border-zinc-100 bg-apple-canvas-parchment p-4">
                <p className="text-sm font-semibold text-apple-ink">{String(plan.planName ?? "")}</p>
                <p className="mt-1 text-lg font-bold text-emerald-600">{revAmount}</p>
                <p className="text-xs text-apple-ink-muted-48">{String(plan.orders ?? 0)} orders</p>
              </div>
            );
          })}
          {byPlan.length === 0 && (
            <p className="col-span-full text-sm text-apple-ink-muted-48">No plan data yet</p>
          )}
        </div>
      </motion.div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          className="rounded-2xl border border-zinc-200 bg-white p-6">
          <h3 className="mb-1 text-lg font-semibold text-apple-ink">Revenue by Payment Method</h3>
          <p className="mb-4 text-sm text-apple-ink-muted-48">Payment method performance in ৳</p>
          <div className="space-y-3">
                ...
                <div key={method} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-semibold text-apple-ink">{revAmount}</span>
                    </div>
            ...
            {byPayment.length === 0 && (
              <p className="text-sm text-apple-ink-muted-48">No payment data yet</p>
            )}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          className="rounded-2xl border border-zinc-200 bg-white p-6">
          <h3 className="mb-1 text-lg font-semibold text-apple-ink">Top Stores by Revenue</h3>
          <p className="mb-4 text-sm text-apple-ink-muted-48">Highest earning stores in ৳</p>
          <div className="space-y-2">
              ...
              return (
                <div key={String(store.storeId ?? "")} className="flex items-center justify-between rounded-lg border border-zinc-100 px-3 py-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-apple-ink truncate">{String(store.storeName ?? "")}</p>
                    <p className="text-xs text-apple-ink-muted-48">{String(store.orders ?? 0)} orders</p>
                  </div>
            ...
            {byStore.length === 0 && (
              <p className="text-sm text-apple-ink-muted-48">No store data yet</p>
            )}
          </div>
        </motion.div>
      </div>
